...
import { dutiesTable, dutiesUsers, usersTable } from '../../drizzle/schema';
...
export class DutiesService {
  ...
  async createDuty(duty: CreateDutyDto) {
    ...
    // 只有当 enabled 为 1 时才添加定时任务
    if (insertedDuty.enabled === 1) {
      try {
        this.task.addCronJob(insertedDuty);
        this.logger.log(
          `值班计划 "${insertedDuty.name}" 创建成功，定时任务已启动`,
        );
      } catch (cronError) {
        this.logger.error(
      ...
      this.logger.log(
        `值班计划 "${insertedDuty.name}" 创建成功，但未启用，未添加定时任务`,
      );
    }
  ...
  async searchDuties(
    ...
    // 为每个 duty 获取当前值班人信息
    const dutiesWithCurrentUser = await Promise.all(
